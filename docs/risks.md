...
| R-08 | **The API matrix is untested.** PRODUCT_SPEC 17.2 asks for API 26, 31, 34 and 36; the suite runs 34. `minSdk` is 26. | API 26 is eight releases below anything exercised. The per-app-language backport (ADR-0022) and the storage-volume code are the two most likely to differ, and both were written against documentation rather than a device. | Robolectric at `sdk = [26, 31, 34, 36]` for the pure-logic screens costs almost nothing and would catch the API-26 half. The rest needs R-07. |
...
| R-10 | **Android Auto has never been run**, in a car or in the Desktop Head Unit. The browse tree, its search, and `onSetMediaItems`' spoken-query path are fully implemented and entirely unexercised. | A driver is the user least able to work around a broken screen. A tree that throws leaves the app listed and dead. | The DHU, which is free and runs on a workstation. PRODUCT_SPEC 17.2 asks for it "where practical". |
...
| R-15 | **The match-candidate fixture disagrees with CI on every run.** Google Books answers `429` to GitHub Actions' address ranges, so the CI capture records an empty list while the committed shape comes from `audiobooks.dev`. | A permanent, expected disagreement is indistinguishable from a real drift the day one appears. | Nothing cheap. Recorded so the next reader does not spend an afternoon on it. |
...
| # | Risk | If it bites | Retired by |
| --- | --- | --- | --- |
| R-17 | **`accountType` defaults to `''` for any install that upgraded through migration 18**, until a sign-in or a permission refresh rewrites it — and `ProfileRole.ofAccountType("")` is `Listener`. | Nothing today: the UI gates on the `role` column, which sign-in writes. It is a loaded gun for the next reader, because gating on `accountType` instead would silently demote every upgraded admin until their next refresh. | Either backfilling it on first launch, or a comment on the column saying which of the two fields a permission check may read. The second is nearly free. |
| R-18 | **Migrations 15, 17, 18 and 19 have no test of their own.** The chain is covered — every test migrates from its starting version all the way to current, so all eighteen steps run — but no test asserts what those four *default their new columns to*. | A wrong default is the failure mode that survives a migration test: the step succeeds, the column exists, and its value is a lie. That is exactly R-17's shape. | Four short tests in the existing harness, which already builds each version from its committed schema export. |
...
| R-29 | **Accessibility is enforced on the JVM and never checked on a device.** `assertEveryControlIsLabelled` and `assertEveryControlIsBigEnough` now walk the whole semantics tree on Settings and the book screen, and one screen renders at a doubled font scale. Three things stay out of reach: whether a label is *useful*, whether contrast passes, and what TalkBack does with the reading order. | TalkBack, large text, touch targets and contrast are release requirements (§51, 2.10). The tree being correct is necessary and not sufficient. | One TalkBack pass on a device, and extending the two assertions to the remaining screens — the player, the shelf, the downloads queue and the metadata editor. |
