...
| --- | --- | --- | --- |
| 1.1 | **Restrict what an arbitrary controller may do with the exported session** | Verified, not assumed: the service is `exported="true"` with no permission attribute, there is no allowlist or `reject` anywhere in `:playback`, and `onConnect` returns `DEFAULT_SESSION_AND_LIBRARY_COMMANDS` plus every custom command to whatever binds. So any app on the device can browse the library tree and drive playback. **Exporting is not the problem** — Android Auto and Assistant need it, and un-exporting would break both. The question is which callers get which commands. The bearer half of this row shipped: a token now reaches only its issuing origin. | Small change, real thinking, plus a second-UID controller test to prove the refusal. |
...
| # | Item | State |
| --- | --- | --- |
| 3.1 | **Excluded tracks resolve against the wrong offsets** (R-22, PLAY-003). A book whose server-side track list excludes a file maps player positions to book positions incorrectly — a resume lands at the wrong point, silently, in a book the user cannot tell is affected. Open since Phase 2. Rare: nearly no book excludes a track. | The fix is described in ADR-0016 — carry `startOffset` into the media item's extras so `PlayerPositions` and `BookMediaSourceFactory` share one coordinate space. |
| 3.2 | **Signing in again destroys a passcode without saying so** (R-44). `SignInUseCase` calls `clearIfLocked`, which is what makes the curtain's recovery route real — but it fires from the ordinary sign-in screen too, where a user may only have meant to refresh an expired session, and nothing on that screen mentions the lock. | Needs a sentence on the sign-in screen, or a narrower trigger. |
...
1. **1.1**, the controller trust decision — it is the only remaining item that is a security exposure
   rather than an unknown, and it is small.
