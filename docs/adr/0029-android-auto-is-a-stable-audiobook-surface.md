...
`MediaButtonSlotConversionTest` executes that conversion from inside Media3's own package, so the claim fails a build instead of sitting in prose — the specific remedy for this section's habit. The slot chain stays, because it is correct for Media3-native controllers, but it no longer claims anything about a car. PLAY-007's skips hold both primary slots, so the bar can show an output action only by giving one of them up; `docs/risks.md` R-109 records that trade as the owner's decision rather than settling it here.
