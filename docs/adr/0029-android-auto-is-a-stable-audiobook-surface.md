...
"Nowhere to play" is **route evidence where the platform offers any and connection evidence where it does not**. Below API 33 `AudioOutputRouter` cannot ask which route is live, and under Automatic routing it has no selection to fall back on either, so it reports every output inactive — an `isActive` test can never pass and the resume was dead from API 26 to 32. The fallback is weaker on purpose and the weakness is the platform's: with no route to read, a connected non-speaker output is the whole of what is knowable, and it still carries the guarantee that matters, because Android does not choose the built-in speaker while another output is connected. What it cannot say is *which* non-speaker, which is R-106's business and a car's to settle.
