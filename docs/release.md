...
| --- | --- | --- |
| Decide whether the exported session's browse tree should be restricted | AUTH-003, PLAY-001, 15 | **Two of three parts done.** The bearer reaches only its issuing origin, and `onAddMediaItems` gates pre-resolved items on the caller's UID so no outside app can inject a URI into the authenticated client. The third is open by decision rather than by oversight: the browse surface is granted to any controller that binds, so any installed app can enumerate the active profile's book titles (14.5) and drive transport. `isThisApplication`'s KDoc argues that is correct — Android Auto, Assistant and headsets need it. Revisiting it needs a policy (which callers get which commands) and a second-UID controller test. |
