...
| --- | --- | --- |
| Restrict what an arbitrary controller may do with the exported session | AUTH-003, PLAY-001, 15 | **Half done.** The bearer half shipped: `AuthorizationInterceptor` attaches a token only to the origin that issued it, and `SessionTokenProvider` carries that origin rather than inheriting whichever was cached last. The controller half has not: `PlaybackService.onConnect` returns `DEFAULT_SESSION_AND_LIBRARY_COMMANDS` plus every custom command to **any** package that binds, so any app on the device can browse the library tree and drive playback. Needs a trust decision (which callers get which commands) and a second-UID controller test. |
