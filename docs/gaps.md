...
| §6.5.6 | **The new profile's last book is not restored paused after a switch.** 6.5 ends by restoring the incoming account's last player state paused, and nothing does: the switch leaves a stopped player and the listener presses play. `ApplyStartupModeUseCase` does this shape of thing on a cold start and is the wrong tool here — it honours `StartupMode`, and `ResumeOnOpen` would *start playing* on a switch, which 6.5.3 and 6.5.8 both forbid. What is missing is a per-profile "last played" query in `:domain` and an unconditional `arm` behind the switch, which also has to wait for `restoreSession` to finish. | Open. Nothing is lost without it; it is a convenience 6.5 asks for, and the boundary half of 6.5 is closed |
