...
| §8.12 | **Closed 2026-08-29 by owner decision — ADR-0028.** Nothing obliges an admin account to take a passcode, and nothing will: *"If the user is logged in as admin, that is enough."* No step-up authentication before privileged operations either. §8 is a list of **recommended** features rather than acceptance criteria, and item 12 is satisfied by offering the lock to every account, admin ones included, which the app does. | **Closed as decided.** The accepted cost is recorded in ADR-0028 and in R-80: somebody holding the unlocked phone with an admin profile active can remove an item from the server database, create a user, and rewrite a book's files unchallenged by this app |
...
| PLAY-001 / notifications | **Closed 2026-08-29 — the row was stale in both halves.** The review that wrote it saw a Debug Console reporting **Blocked**, and concluded there was no path. There is: `NotificationPermission` in `MainActivity` launches the `POST_NOTIFICATIONS` request the first time a book plays — deliberately then rather than at launch, because *"so you can pause the book from your lock screen"* is only true once there is a book — and `NotificationBlockedNotice` in the player carries a button straight to `ACTION_APP_NOTIFICATION_SETTINGS` for the case where it was already refused. Verified by reading both call sites on 2026-08-29 while building the output chooser. | **Closed.** What remains true is that a refusal costs the notification and not the audio, which is stated in the KDoc and is by design |
...
| LIB-003 | **Mostly closed 2026-08-29.** The series screen now opens on a header: the cover of the book you would carry on with, *"N of M books finished"*, the series' total running time, and one button that starts the first **unfinished** book — not the first started one, so a series read in order offers book three while one and two are done and three has never been opened. A finished series says so and offers no button, because restarting one on a single tap is not a thing to leave under a thumb. Every row carries its own play control, which `BookCard` takes as an optional callback so no other shelf grows one. | **The summary is still missing and is not buildable today.** `Series` is `(serverId, id, name)` and carries no description; the first book's would describe the wrong thing, and reading a real one means a new API field and a contract capture first (22.4). The series name is deliberately *not* repeated in the header — it is in the top bar, and duplicating it is the defect the row below records against the book detail. | **Closed but for the summary.** `SeriesHeaderTest` holds the arithmetic, both play paths and the doubled-font-scale layout; two mutations were run against it |
...
| PLAY-001 / LIB-002 | **Closed 2026-09-08 — the car browse root is Continue / Series / Authors / Library.** The gap described a root of Continue/Chapters/History that offered no library browse axes. The owner asked for the swap after driving with it: Series and Authors took the two positions Chapters and History held, and those two moved to the top of Library rather than being removed. Downloads and Genres are still not root destinations, and cannot be — Android Auto's documented root-children limit is four, so the root is at its ceiling and Downloads remains one level down inside Library. | Done. The depth/distraction cost this row wanted measured in the DHU was decided by the owner driving with the previous layout instead, which is better evidence. |
