...
| MGR-003 | **A populated candidate list cannot be captured from CI.** Google Books answers `429` to GitHub Actions' addresses, every run, so `search-books-shape.json` records an empty result. The endpoint works; capturing a real result needs a run from somewhere else. | Open, environmental |
