...
class PlaybackService : MediaLibraryService() {
     ...
     *
     * Written by the service's player/session transitions and read by the shared resume-freshness
     * coordinator. A singleton, so both are operating on the same acknowledgement record — see
     * [ResumeBaseline].
     */
    @Inject
    ...
    private suspend fun openQueue(
        ...
        is AppResult.Success -> {
            val openedSession = opened.value
            bookChanges.onBookOpened(openedSession, initialPlayWillFollow = initialPlayWillFollow)
    ...
    private companion object {
        ...
        /**
         * How long an adopted seek has to report back before it is treated as lost.
         *
         * A failed confirmation leaves the book paused. There is deliberately no second `/play` recovery:
         * reopening a server session after ownership has changed is a state-changing transaction that cannot
         * be made safe by a late generation check.
         */
