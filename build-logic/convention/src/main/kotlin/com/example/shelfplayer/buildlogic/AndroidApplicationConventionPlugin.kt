...
private fun ApplicationExtension.configureBuildTypes() {
    buildTypes {
         ...
         *
         *  - **It is signed with the debug key.** A release build here declares no signing config at all
         *    (PRODUCT_SPEC 15) and an unsigned APK cannot be installed, so it could not be measured. The
         *    debug keystore is generated by the SDK on the machine running the build and is not in this
         *    repository, so this adds no key material to it.
